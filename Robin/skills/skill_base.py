...
Rutas de escritura permitidas para Robin:
  - /app/Robin/reportes/    (entregables)
...
_AGENTE   = "Robin"
_TRABAJO  = _APP_ROOT / _AGENTE / "reportes"
...
def _ruta_es_valida(ruta_str: str) -> tuple[bool, str]:
    ...
    if ruta_norm.startswith(raiz_agente):
        # Subcarpetas permitidas dentro de /app/Robin/
        subcarpetas_ok = ["reportes", "skills", ".agents"]
...
def _msg_hard_stop(ruta: str) -> str:
    return (
        ...
        f"  /app/{_AGENTE}/{"reportes"}/  → entregables de trabajo\n"
...
def crear_archivo(ruta_absoluta: str, contenido: str) -> str:
    ...
    try:
        ...
        if ruta_str.endswith(".md"):
            ...
            elif f"/{_AGENTE}/{"reportes"}/" in ruta_str or ruta_str.endswith(f"/{_AGENTE}/{"reportes"}"):
