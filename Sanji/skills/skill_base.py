...
Rutas de escritura permitidas para Sanji:
  - /app/Sanji/documentos_sanji/    (entregables)
...
_AGENTE   = "Sanji"
_TRABAJO  = _APP_ROOT / _AGENTE / "documentos_sanji"
...
def _ruta_es_valida(ruta_str: str) -> tuple[bool, str]:
    ...
    if ruta_norm.startswith(raiz_agente):
        # Subcarpetas permitidas dentro de /app/Sanji/
        subcarpetas_ok = ["documentos_sanji", "skills", ".agents"]
...
def _msg_hard_stop(ruta: str) -> str:
    return (
        ...
        f"  /app/{_AGENTE}/{"documentos_sanji"}/  → entregables de trabajo\n"
...
def crear_archivo(ruta_absoluta: str, contenido: str) -> str:
    ...
    try:
        ...
        if ruta_str.endswith(".md"):
            ...
            elif f"/{_AGENTE}/{"documentos_sanji"}/" in ruta_str or ruta_str.endswith(f"/{_AGENTE}/{"documentos_sanji"}"):
